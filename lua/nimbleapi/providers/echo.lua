...
--- HTTP method lookup table for Echo route shortcuts.
--- Keys are Go method names; values are normalized HTTP method strings.
--- Echo has 9 method shortcuts (vs Gin's 7): adds CONNECT and TRACE.
...
--- Detect if this is an Echo project.
---@param root string
---@return boolean
function M.detect(root)
  local gomod = utils.join(root, "go.mod")
  if utils.file_exists(gomod) then
    if utils.file_contains(gomod, "github.com/labstack/echo") then
      return true
    end
  end
  return false
...
--- Find the app entry point (stub — Echo does not need an entry point for route scanning).
---@param root string
---@return table|nil
...
--- Pass 1: Collect Group variable assignments (echo-groups.scm).
--- Pass 2: Extract route calls and resolve their full prefix chains (echo-routes.scm).
...
function M.extract_routes(filepath)
  ...
  -- Pass 1: Collect all Group variable assignments in this file.
  ...
  local groups = {}
  local ok_groups, groups_query = pcall(parser.get_query_public, "echo-groups", "go")
  ...
  local routes = {}
  local ok_routes, routes_query = pcall(parser.get_query_public, "echo-routes", "go")
  ...
  for _, match, _ in routes_query:iter_matches(root_node, source, 0, -1) do
    ...
    for id, nodes in pairs(match) do
      ...
      elseif name == "_add_method" then
        add_method_text = strip_quotes(parser.get_text(node, source))
      elseif name == "route_path" then
        route_path_text = strip_quotes(parser.get_text(node, source))
      elseif name == "func_name" then
        -- CRITICAL: func_literal guard — anonymous inline handlers produce multi-line body text
        -- that crashes nvim_buf_set_lines with E315: ml_get: invalid lnum (Gin Phase 2 bug #1)
    ...
    local method = nil
    if http_method_text == "Add" then
      -- e.Add("GET", "/path", handler) — method is the first string argument (D-05)
      if add_method_text then
        method = add_method_text:upper()
      end
    else
      -- Method shortcut (GET, POST, PUT, DELETE, PATCH, HEAD, OPTIONS, CONNECT, TRACE, Any)
      method = ECHO_METHODS[http_method_text]
    end

    -- Skip if method not recognized (filters out non-route calls like .Use(), .Static(), etc.)
    if method and route_path_text and func_name_text ~= nil and router_obj_text then
...
--- Get all routes across all Go files in an Echo project.
--- Scans all *.go files (excluding vendor, testdata, node_modules, .git),
--- pre-filtering to files that likely contain Echo route registrations.
...
function M.get_all_routes(root)
  ...
  for _, f in ipairs(go_files) do
    -- Pre-filter: only parse files that likely contain Echo route or group calls.
    -- "echo." catches echo.New(), echo.Echo — the engine setup.
    -- The method calls catch files using a router variable without "echo." import reference.
    if
      utils.file_contains(f, "echo.")
...
--- Build the route tree for the Echo project (simple wrapper over get_all_routes).
---@param root string
---@return table|nil
...
--- Echo has no cross-file router composition (unlike FastAPI's include_router),
...
--- Handles both string literal ("GET") and http.MethodXxx constant forms for the method argument.
---@param bufnr integer
